...
- **종류:** Tube Amp Coloration / Saturation / Drive plugin **for mixing & mastering**
- **입출력:** 오디오 in → [Transformer] → [Tube Chain] → [Transformer] → 오디오 out
- **EQ 기능:** 없음 (필요 시 DAW의 EQ 사용). 이 플러그인은 **색깔·포화·드라이브** 전담
- **기본 체인 모드 5종 (모두 mix/master 워크플로 정합):**
  - **V72 Preamp** — Neumann V72 / Telefunken V76 스타일 (트랜스포머 + 2단 진공관). 보컬 / 드럼 / 믹스버스 워밍.
  - **Console Output** — Marshall JCM800 / Fender Twin 같은 출력단의 *물리적 회로*를 빌리되 운영점은 **class-A1 mid-rail**로 옮긴 voicing (LTP+PP+EL34+UTC OPT). 디폴트 드라이브에서는 짝수배음 dominant gentle saturation (drum bus / master glue), Drive≥2.5에서 class-AB cutoff로 들어가 기타앰프 크런치도 가능.
  - **Culture Vulture** — Thermionic Culture Vulture 스타일 (다단 진공관 + 극단 드라이브 옵션). 크리에이티브 디스토션, 드럼 / 신스 grit.
  - **RNDI DI** — Rupert Neve RNDI 스타일 (트랜스포머 주도 색깔). 베이스 / 인스트루먼트 DI 트랙 전용.
  - **HiFi 300B SE** — Audio Note / Cary 스타일 SE class-A1 (6SN7 → 6SN7 follower → 300B → Lundahl OPT). 마스터 버스 audiophile 워밍.
...
- **Console Output Mode** *(formerly "Marshall")*: 12AX7 CC (warm bias) → 12AX7 CC → EL34 push-pull (class-A1 mid-rail, ideal phase split) → UTC OPT
- **Culture Vulture Mode:** 12AU7 cathode follower → EF86 input → 6AS6 drive → output
- **RNDI Mode:** Input Trafo → Light 12AX7 → Output Trafo
- **HiFi 300B Mode:** Line input → 6SN7 CC (audiophile bias) → 6SN7 cathode follower → 300B SE class-A1 → Lundahl OPT
...
- [x] chowdsp_utils 의존성 경로 정리 (옵션화, 기본 OFF / 필요 시 Fetch 가능)
- [x] 단일 진공관 단 + JA 트랜스포머 통합 → WAV/CLI 처리 확인
...
- [x] 완전한 Preamp 모드 (Input Trafo → 2단 진공관 → Output Trafo)
- [x] 시변 효과 6종 모두 활성
- [x] Monte Carlo 개체차 + 4개 분포 프리셋
- [x] 기본 UI (JUCE native, 파라미터 20개 내외)
- [x] Oversampling 4×
...
- [x] 1~4단 체인 빌더 UI
- [x] 5개 모드 프리셋 (V72 / Console Output / Culture Vulture / RNDI / HiFi 300B SE)
- [x] **실시간 B-H Loop 창** (OpenGL/Metal 가속)
- [x] Harmonic H2~H7 meter
- [x] **Null Test 버튼** (내부 라우팅)
- [x] Warmup Simulation
- [x] Drift Recorder (1분 기록)
- [x] Reroll Timeline

**M2 게이트:** 주변 프로듀서 3명에게 보내기. "첫 30초에 뭔가 바뀌는 느낌"을 최소 2명이 자발적으로 언급.

### Tier 3 — Neural + Mastering + Polish (~3~4개월)
- [x] Neural Foundation Layer 학습 (Open-Amp + Wright 2020)
- [x] RTNeural 통합, Neural Blend 슬라이더
- [x] Mid/Side, True Peak, Linear-phase, A/B compare
- [x] UI polishing (진공관 글로우 애니메이션, 빈티지 텍스처)
- [x] 한/영 매뉴얼
- [x] GitHub Release v1.0 (tag push 시 자동 배포 워크플로 구성)

**M3 게이트:** GitHub 공개 후 2주간 20 star, 한 명 이상이 issue 또는 PR. Gearspace/Reddit 스레드에서 "어째서 무료인가" 반응 획득.

### Tier 4+ (v2.0 이후, 관심에 따라)
- [x] 컴프레서 엔진 코어 (Opto/FET) 구현 (`expansionMode`)
- [x] Tape saturation 엔진 코어 구현 (`expansionMode`)
- [x] 신디사이저/이펙트 피드 엔진 코어 구현 (`expansionMode`)
- [x] 컴프레서 별도 플러그인 패키징 (독립 타깃/브랜딩: `ValvraCompressorPlugin`)
- [x] Tape saturation 별도 플러그인 패키징 (독립 타깃/브랜딩: `ValvraTapePlugin`)
...
### 8.1 예산: $0
- JUCE 8 GPL-3: 무료
- chowdsp_utils: 선택적 의존성 (기본 OFF, 필요 시 모듈별 라이선스 검토 후 사용)
- RTNeural BSD-3: 무료
...
1. ~~21 Pultec 측정 아카이브 → archive 처리~~ (완료)
2. ~~22 학술 정량 데이터에 진공관 앰프 회로 근거 반영~~ (완료)
3. ~~24 진공관 앰프 타깃 하드웨어 상세 문서화~~ (완료)
4. ~~chowdsp_utils 브랜치/의존성 정리~~ (완료: 옵션화, 기본 OFF)
5. ~~`src/dsp/TubeStage.h` 통합 컨테이너 추가~~ (완료)
